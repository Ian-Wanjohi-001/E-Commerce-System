...
export const updateCart = async (req, res) => {
    ...
    try {
      ...
      const result = await pool.request()
        ...
        .input('Name', sql.VarChar(255), Name)
        .input('Price', sql.Decimal(10, 2), Price)
        .query('UPDATE Cart SET ProductID = @ProductID, Quantity = @Quantity, CustomerID = @CustomerID, Name = @Name, Price = @Price WHERE CartID = @cartItemId');
  ...
  export const deleteCart = async (req, res) => {
    try {
      ...
      const result = await pool.request()
        .input('cartItemId', sql.Int, cartItemId)
        .query('DELETE FROM Cart WHERE CartID = @cartItemId');
